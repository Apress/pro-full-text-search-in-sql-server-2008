...
namespace Tst
{
		...
		,ICloneable
	{
		...
		public virtual ICollection NearNeighbors(string key, int distance)
		{
			...
			ArrayList matches = new ArrayList();
			NearNeighborsSearch(Root, key, 0, distance, matches, distance);
			return matches;
		}
		...
		{
			if (p == null || dist < 0 || index == key.Length) 
				return;
				
			char c = key[index];
			// low child
			if (dist > 0 || c < p.SplitChar)
				NearNeighborsSearch(p.LowChild, key, index, dist, matches, offset);

			// eq child
            int next_dist = (key[index] == p.SplitChar) ? dist : dist - 1;

			if (p.IsKey && (key.Length - index) <= next_dist) 
			{
				matches.Add(new DictionaryEntry(p.Key, offset - next_dist));
			}
			else
			{
            ...
            // highchild
			if (dist > 0 || c > p.SplitChar)
				NearNeighborsSearch(p.HighChild, key, index, dist, matches, offset);
		}
	}
}
